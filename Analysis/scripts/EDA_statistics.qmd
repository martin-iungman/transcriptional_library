...
theme_set(theme_bw())
files=list.files("Data_processing/Count_table","counts_ampliconseq2023_f2_.-..tsv", full.names = T)
...
names(data)=files%>%str_remove("^.+2023_f2_")%>%str_remove(".tsv$")
...
perc_cells%>%ggplot(aes(sample, perc))+geom_col(fill=clrs[2])+xlab("Gate")+ylab("Sample effort relativization factor")
```
