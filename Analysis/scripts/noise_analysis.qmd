...
data  %>%  ungroup() %>% group_split(rep) %>% 
  ...
  ggplot(aes(FPR,TPR, col=rep))+geom_line()+geom_abline(linetype="dashed")+
  geom_text(data=.%>% select(AUC, rep) %>% mutate(AUC=paste("AUC:", round(AUC,3)))%>% unique() %>% bind_cols(tibble(TPR=c(0.9,0.8), FPR=0.02)), aes(label=AUC), hjust="left")+ggtitle("TATA-box ROC curve")+scale_color_manual(values=c("#0D2C54", "#AD343E"))+theme_pubr(base_size = 20)+labs(linetype="", col="")
...
data  %>%  ungroup() %>% group_split(rep, as.numeric(mean_sw)>70) %>% 
    ...
    geom_text(data=.%>% select(AUC, rep,gr) %>% mutate(AUC=paste("AUC:", round(AUC,3)))%>% unique() %>% bind_cols(tibble(TPR=c(0.9,0.8, 0.7,0.6), FPR=0.02)), aes(label=AUC), hjust="left")+ggtitle("TATA-box ROC curve")+scale_color_manual(values=c("#0D2C54", "#AD343E"))+theme_pubr()+labs(linetype="", col="")
