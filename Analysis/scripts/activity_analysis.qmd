...
data %>% select(seq_id, mean, rep) %>% pivot_wider(values_from = "mean", names_from = "rep") %>% summarize(corre=cor(`Rep 1`, `Rep 2`, use="complete.obs", method="spearman"))
```
...
ggsave("Plots/Fig3/HEK_mean.pdf")
...
ggsave("Plots/Fig2/CGI_scatter.pdf", create.dir = T)
...
```{r}
p=ggviolin(data %>% filter(rep=="Rep 1"), x="CGI", y="mean", fill="CGI", draw_quantiles = 0.5, add="median_q1q3", palette=c("#14AFB2", "#216869"), alpha=0.7)
p+stat_compare_means()+theme_pubclean()+labs(fill="Cpg island", x=NULL, y="Transcriptional mean activity")+theme(axis.text.x = element_blank(), axis.ticks.x = element_blank())+ylim(c(0.5,6.2))
ggsave("Plots/Fig2/CGI_violin_rep1.pdf")
p=ggviolin(data %>% filter(rep=="Rep 2"), x="CGI", y="mean", fill="CGI", draw_quantiles = 0.5, add="median_q1q3", palette=c("#14AFB2", "#216869"), alpha=0.7)
p+stat_compare_means()+theme_pubclean()+labs(fill="CpG isalnd", x=NULL, y="Transcriptional mean activity")+theme(axis.text.x = element_blank(), axis.ticks.x = element_blank())+ylim(c(0.5,6.2))
ggsave("Plots/Fig2/CGI_violin_rep2.pdf")
```
...
ggsave("Plots/Fig2/TATA_scatter.pdf")
```

```{r}
p=ggviolin(data %>% filter(rep=="Rep 1"), x="TATA_EPD", y="mean", fill="TATA_EPD", draw_quantiles = 0.5, add="median_q1q3", palette=c("#14AFB2", "#216869"), alpha=0.7)
p+stat_compare_means()+theme_pubclean()+labs(fill="TATA-box", x=NULL, y="Transcriptional mean activity")+theme(axis.text.x = element_blank(), axis.ticks.x = element_blank())+ylim(c(0.5,6.2))
ggsave("Plots/Fig2/TATA_violin_rep1.pdf")
p=ggviolin(data %>% filter(rep=="Rep 2"), x="TATA_EPD", y="mean", fill="TATA_EPD", draw_quantiles = 0.5, add="median_q1q3", palette=c("#14AFB2", "#216869"), alpha=0.7)
p+stat_compare_means()+theme_pubclean()+labs(fill="TATA-box", x=NULL, y="Transcriptional mean activity")+theme(axis.text.x = element_blank(), axis.ticks.x = element_blank())+ylim(c(0.5,6.2))
ggsave("Plots/Fig2/TATA_violin_rep2.pdf")
```
...
```{r}
remap=read_tsv("Analysis/Tables/allPeaks_light.hg38.50_lib_final.tsv")

remap_scatter_plot=function(tf){
  data %>% mutate(tf=seq_id%in%(remap %>% filter(feature==tf))$seq_id) %>% 
    group_by(mean_sw,rep) %>% summarise(tf=sum(tf)/n()) %>% ggplot(aes(mean_sw,tf))+geom_point(col="#14AFB2")+
    geom_smooth(col="#216869")+theme(text=element_text(size=20))+facet_wrap(~rep)+
    ylim(0,1)+labs(x="Mean activity (binned by rank)",y="Proportion of promoters")+ggtitle(paste(tf,"binding")) 
}

remap_scatter_plot("NFYA")
ggsave("Plots/Fig2/NFYA_scatter.pdf", create.dir = T)

remap_scatter_plot("SP1")
ggsave("Plots/Fig2/SP1_scatter.pdf", create.dir = T)

remap_scatter_plot("SP2")
ggsave("Plots/Fig2/SP2_scatter.pdf", create.dir = T)

# remap_scatter_plot("SP3")
# ggsave("Plots/Fig2/SP3_scatter.pdf", create.dir = T)
...
```{r}
dudnyk_pred_effects=read_tsv("Analysis/Tables/Dudnyk_puffin_prediction_summ.tsv")
...
```{r}
data %>% group_by(rep) %>% filter(!is.na(selectivity_dudnik)) %>% mutate(mean_sw=row_number(mean) %>% cut_number(100) %>% as.numeric()) %>% group_by(rep, mean_sw)%>%  summarise(puffin=median(selectivity_dudnik, na.rm=T)) %>% ggplot(aes(mean_sw,puffin))+geom_point(col="#14AFB2")+geom_smooth(col="#216869")+facet_wrap(~rep)+labs(x="Mean activity (binned by rank)",y="Selectivity index")+ggtitle("PUFFIN selectivity")+theme_pubr()
ggsave("Plots/Fig3/PUFFIN_selectivity_scatter.pdf")

data %>% group_by(mean_sw,rep) %>% summarise(puffin=median(teoTSS_score)) %>% ggplot(aes(mean_sw,puffin))+geom_point(col="#14AFB2")+geom_smooth(col="#216869")+theme(text=element_text(size=20))+facet_wrap(~rep)+labs(x="Mean activity (binned by rank)",y="Median prediction score")+ggtitle("PUFFIN prediction over TSS position")
ggsave("Plots/Fig3/PUFFIN_predscore_scatter.pdf", create.dir = T)
```
...
```{r}
data  %>% mutate(high_sp=gini_tissue>median(gini_tissue, na.rm=T)) %>% filter(!is.na(high_sp)) %>% group_by(mean_sw,rep) %>% summarise(high_sp=sum(high_sp)/n()) %>% ggplot(aes(mean_sw,high_sp))+geom_point(col="#14AFB2")+geom_smooth(col="#216869")+theme(text=element_text(size=20))+facet_wrap(~rep)+ylim(0,1)+labs(x="Mean activity (binned by rank)",y="Proportion of promoters")+ggtitle("High tissue specificity")
ggsave("Plots/Fig2/high_tissue_sp_scatter.pdf")
```

```{r}
p=ggviolin(data %>% filter(rep=="Rep 1")  %>% mutate(high_sp=gini_tissue>median(gini_tissue, na.rm=T)) %>% filter(!is.na(high_sp)), x="high_sp", y="mean", fill="high_sp", draw_quantiles = 0.5, add="median_q1q3", palette=c("#14AFB2", "#216869"), alpha=0.7)
p+stat_compare_means()+theme_pubclean()+labs(fill="High tissue specificity", x=NULL, y="Transcriptional mean activity")+theme(axis.text.x = element_blank(), axis.ticks.x = element_blank())+ylim(c(0.5,6.2))
ggsave("Plots/Fig2/high_tissue_sp_violin_rep1.pdf")
p=ggviolin(data %>% filter(rep=="Rep 2") %>% mutate(high_sp=gini_tissue>median(gini_tissue, na.rm=T)) %>% filter(!is.na(high_sp)), x="high_sp", y="mean", fill="high_sp", draw_quantiles = 0.5, add="median_q1q3", palette=c("#14AFB2", "#216869"), alpha=0.7)
p+stat_compare_means()+theme_pubclean()+labs(fill="High tissue specificity", x=NULL, y="Transcriptional mean activity")+theme(axis.text.x = element_blank(), axis.ticks.x = element_blank())+ylim(c(0.5,6.2))
ggsave("Plots/Fig2/high_tissue_sp_violin_rep2.pdf")
```
...
tidy_data=data  %>% select(-tissue_enrichment)%>% fastDummies::dummy_cols("TE_superclass", ignore_na = T, omit_colname_prefix=T, remove_selected_columns = T) %>% fastDummies::dummy_cols("expresion_vbility_class_tissue", ignore_na = T, omit_colname_prefix=T, remove_selected_columns = T) %>% 
  mutate(`High G+C content`=g_c>median(g_c),
         across(c(DNA, SINE, LINE,LTR), ~replace_na(.x, 0)),
                          ...
                          shape_tissue=cut_number(shape_tissue, n=3) %>% as.numeric(),
                          
                          `Narrow promoters`=shape_tissue==1,
                          `Broad promoters`=shape_tissue==3,
