...
ggplot() + geom_point(data=df_hist, aes(mean, mean_density, col=seq_id), size=2.5)+
    ...
    labs(x="Mean (Histogram)", y="Mean (Density)")+theme(text=element_text(size=20))+labs(col="Promoter")+scale_color_manual(values = c(KIAA0753_1="#AD343E", TMEM87A_1="#FFB400", ))
```
...
ggsave("Plots/Fig1/transient_validation_hist.png", p)
```
...
ggsave("Plots/Fig1/transient_validation_density.png", p)
```
